...
export default function Dashboard() {
  ...
  const statCards = [
    { title: 'Total Saldo Kas', value: `Rp ${stats.kas.toLocaleString('id-ID')}`, icon: <DollarSign size={28} className="text-emerald-500" />, bg: 'bg-emerald-100', textCol: 'text-emerald-900', border: 'border-emerald-200' },
    { title: 'Total Pendapatan', value: `Rp ${stats.pendapatan.toLocaleString('id-ID')}`, icon: <TrendingUp size={28} className="text-primary-500" />, bg: 'bg-primary-100', textCol: 'text-primary-900', border: 'border-primary-200' },
    { title: 'Macam Barang/Stok', value: `${stats.stokCount} Item`, icon: <Package size={28} className="text-amber-500" />, bg: 'bg-amber-100', textCol: 'text-amber-900', border: 'border-amber-200' },
    { title: 'Jumlah Transaksi', value: stats.transaksiCount.toString(), icon: <Users size={28} className="text-sky-500" />, bg: 'bg-sky-100', textCol: 'text-sky-900', border: 'border-sky-200' },
  ];
    ...
    <div className="space-y-6 pb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
        {statCards.map((stat, i) => (
          <div key={i} className={`bg-white rounded-2xl p-6 border ${stat.border} shadow-sm hover:shadow-md trans-all hover:-translate-y-1 relative overflow-hidden group`}>
            ...
            {/* Indikator statis */}
            <div className="mt-4 flex items-center text-[11px] font-bold text-slate-400 bg-slate-50 inline-block px-2 py-1 rounded-md">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 glass rounded-2xl p-6 h-[300px] flex flex-col items-center justify-center text-center">
          ...
          <h3 className="font-bold text-slate-800 text-xl">Grafik Pertumbuhan</h3>
        ...
        <div className="glass rounded-2xl p-6">
          <h3 className="font-extrabold text-slate-800 mb-6 text-lg border-b border-slate-100 pb-3">Status Sistem</h3>
          <div className="space-y-4">
            {['Sistem Keamanan Login', 'Sinkronisasi Database', 'Modul Akuntansi'].map((modul, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-slate-50/80 rounded-xl border border-slate-100 hover:border-emerald-200 trans-all">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-slate-800 text-sm">{modul}</h4>
                    <p className="text-xs text-slate-500 font-medium">Berjalan Normal</p>
                  </div>
                </div>
              </div>
            ))}
