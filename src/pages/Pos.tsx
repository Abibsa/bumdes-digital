...
export default function Pos() {
  ...
  return (
    <>
      {/* AREA KASIR UTAMA (Disembunyikan saat mode Print) */}
      <div className="flex flex-col xl:flex-row gap-6 h-[calc(100vh-130px)] print:hidden">
        ...
        <div className="w-full xl:w-[400px] flex-shrink-0 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col">
          ...
          <div className="p-6 border-t border-slate-200 bg-slate-50 rounded-b-2xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button disabled={cart.length === 0 || loading} onClick={() => handleCheckout(true)} className="flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-800 text-white font-medium hover:bg-slate-900 transition-colors disabled:opacity-50">
                <Printer size={18} /> Simpan & Cetak
              </button>
              <button disabled={cart.length === 0 || loading} onClick={() => handleCheckout(false)} className="flex items-center justify-center gap-2 py-3 rounded-xl bg-primary-600 text-white font-medium hover:bg-primary-700 transition-colors disabled:opacity-50">
      ...
      {/* AREA STRUK (Disembunyikan di layar web, Muncul hanya saat Print) */}
